...
int flippingMatrix(int matrix_rows, int matrix_columns, int** matrix) {
    ...
    for(int col=0;col<matrix_columns;col++)
    {
        ...
        for (int j = n; j < matrix_rows; j++)
            sum2 += matrix[j][col];
            
        if ( sum2 > sum1) // Flip column
        {
            printf("Flipping col %d\n", col); 
            for(int j=0, k=matrix_rows-1 ; j<n; j++, k--)
            {
                int temp = matrix[j][col];
                matrix[j][col] = matrix[k][col];
                matrix[k][col] = temp;
            }
        }
    }
    ...
    for(int row=0;row<matrix_rows;row++)
    {
        ...
        if ( sum2 > sum1) // Flip row
        {
            printf("Flipping row %d\n", row); 
            for(int j=0, k=matrix_rows-1 ; j<n; j++, k--)
            {
                int temp = matrix[row][j];
                matrix[row][j] = matrix[row][k];
                matrix[row][k] = temp;
            }
        }
    }
